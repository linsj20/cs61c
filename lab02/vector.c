...
void vector_set(vector_t *v, size_t loc, int value) {
     ...
     */
    size_t new_size=v->size,origin_size=v->size;
    while(loc>=new_size)new_size<<=1;
    if(new_size>origin_size){
        int* old_data=v->data;
        v->data=malloc(sizeof(int)*new_size);
        for(size_t i=0;i<origin_size;i++)v->data[i]=old_data[i];
        for(size_t i=origin_size;i<new_size;i++)v->data[i]=0;
        free(old_data);
    }
    v->size=new_size;
    v->data[loc]=value;
