...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i+=4){
            ...
            res_vec = _mm_add_epi32(res_vec, _mm_and_si128(mask, tmp));
            _mm_storeu_si128((__m128i*) ress, res_vec);
            result = result + ress[0] + ress[1] + ress[2] + ress[3];
            res_vec = _mm_setzero_si128();
        }       
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        __m128i mask = _mm_setzero_si128();
        int ress[4];
        for(unsigned int i = 0; i < (NUM_ELEMS >> 4) * 16; i+=16){
            tmp = _mm_loadu_si128((__m128i*)(vals + i));
            mask = _mm_cmpgt_epi32(tmp, _127);
            res_vec = _mm_add_epi32(res_vec, _mm_and_si128(mask, tmp));
            _mm_storeu_si128((__m128i*) ress, res_vec);
        ...
        /* Hint: you'll need a tail case. */
        for(unsigned int i = (NUM_ELEMS >> 4) * 16; i < NUM_ELEMS; i++) {
            if (vals[i] >= 128) {
                result += vals[i];
            }
        }
